...
def show_bio(artist_id):
    mycursor.execute("""
    SELECT Artist_Bios.url FROM Artists
    INNER JOIN Artist_Bios ON Artists.bio_id = Artist_Bios.id
    WHERE Artists.id = %d
    """ % artist_id)
...
def show_author(song_id):
    mycursor.execute("""
    SELECT written_by FROM Songs
    WHERE id = %d
    """ % song_id)
